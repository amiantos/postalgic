...
struct PostalgicTests {
    ...
    func createTestBlog() -> Blog {
        ...
        let post1 = Post(
            title: "First Post",
            content: "This is the first post with some **bold** and *italic* text.",
        ...
        let post2 = Post(
            ...
            content: "This is a post without a title, which should use content as the title.",
            createdAt: Date()
        )
    ...
    @Test func testStaticSiteGenerator() async throws {
        ...
        let extractDir = FileManager.default.temporaryDirectory.appendingPathComponent("extract-test-\(UUID().uuidString)")
        ...
        #expect(!siteDirectories.isEmpty, "Should have at least one site directory")
        let siteDir = siteDirectories.first!
        
        // Verify basic structure
        #expect(FileManager.default.fileExists(atPath: siteDir.appendingPathComponent("index.html").path))
        #expect(FileManager.default.fileExists(atPath: siteDir.appendingPathComponent("archives.html").path))
        #expect(FileManager.default.fileExists(atPath: siteDir.appendingPathComponent("css/style.css").path))
        
        // Verify post directories exist
        for post in blog.posts {
            let postPath = siteDir.appendingPathComponent(post.urlPath).appendingPathComponent("index.html").path
    ...
    @Test func testMarkdownInPostContent() async throws {
        ...
        let extractDir = FileManager.default.temporaryDirectory.appendingPathComponent("md-test-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: extractDir, withIntermediateDirectories: true)
        ...
        let postPath = siteDir.appendingPathComponent(post.urlPath).appendingPathComponent("index.html")
        
        // Read the HTML content
        let postHtml = try String(contentsOf: postPath)
        
        // Check for formatted markdown elements
        #expect(postHtml.contains("<strong>bold text</strong>"), "HTML should contain formatted bold text")
        #expect(postHtml.contains("<em>italic text</em>"), "HTML should contain formatted italic text")
        #expect(postHtml.contains("<a href=\"https://example.com\">link</a>"), "HTML should contain formatted link")
        #expect(postHtml.contains("<br>This is a new paragraph."), "HTML should format paragraphs with line breaks")
