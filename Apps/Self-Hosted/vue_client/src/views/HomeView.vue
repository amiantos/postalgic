...
<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <main class="max-w-4xl mx-auto px-4 py-8">
      ...
      <!-- Error -->
      <div v-else-if="blogStore.error" class="bg-red-50 border border-red-200 rounded-lg p-4">
        <p class="text-red-800">{{ blogStore.error }}</p>
      </div>
      ...
      <div v-else-if="blogStore.blogs.length === 0" class="text-center py-12">
        ...
        <h2 class="text-lg font-medium text-gray-900 mb-2">No blogs yet</h2>
        <p class="text-gray-500 mb-6">Create your first blog to get started.</p>
        <router-link
          to="/blogs/new"
      ...
      <div v-else class="space-y-4">
        ...
        >
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <h2 class="text-xl font-semibold text-gray-900">{{ blog.name }}</h2>
              <p v-if="blog.tagline" class="text-gray-500 mt-1">{{ blog.tagline }}</p>
              <p v-if="blog.url" class="text-primary-600 text-sm mt-2">{{ blog.url }}</p>
            </div>
            <button
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Delete Blog</h3>
