...
<template>
  <div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-xl font-bold text-gray-900">Sidebar</h2>
        <p class="text-gray-500 text-sm">{{ blogStore.sidebarObjects.length }} items</p>
      </div>
    ...
    <!-- Empty State -->
    <div v-if="blogStore.sidebarObjects.length === 0" class="text-center py-12 bg-white rounded-lg border border-gray-200">
      <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
        <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z" />
        </svg>
      </div>
      <h3 class="text-lg font-medium text-gray-900 mb-2">No sidebar content yet</h3>
      <p class="text-gray-500 mb-6">Add text blocks or link lists to your blog's sidebar.</p>
    </div>
        ...
        <div class="flex items-start justify-between">
          <div class="flex-1">
            <div class="flex items-center gap-2 mb-2">
              <h3 class="font-medium text-gray-900">{{ obj.title }}</h3>
              <span class="px-2 py-0.5 text-xs bg-gray-100 text-gray-600 rounded-full">
                {{ obj.type === 'text' ? 'Text' : 'Links' }}
              </span>
            </div>
            <div v-if="obj.type === 'text'" class="text-sm text-gray-500 line-clamp-2">
              {{ obj.content }}
            </div>
